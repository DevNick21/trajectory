// Wizard stages, rendered one at a time by OnboardingWizard.
...
function applyCVImportToAnswers(
  ...
  //   1. What the user has already typed
  //   2. The parser-generated narrative bio
  ...
  if (!narrative.trim()) {
    if (imp.narrative && imp.narrative.trim()) {
      narrative = imp.narrative.trim();
    } else if (imp.roles.length > 0) {
      narrative = imp.roles
        .slice(0, 3)
        .map(
          (r) =>
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        ...
        history, skills, and projects — you review and edit on the next
