...
import type { Citation, VerdictLabel } from "@/lib/types";
...
interface VerdictData {
  ...
  hard_blockers?: Array<{ type?: string; detail?: string; citation?: Citation }>;
  stretch_concerns?: Array<{ type?: string; detail?: string; citation?: Citation }>;
}
...
export default function VerdictCard({ verdict, bundle, sessionId }: Props) {
  ...
  const Wrapper = isGo || isBlocked ? AnimatedBorderCard : "div";
  const wrapperProps = isGo
    ? { colorFrom: "hsl(var(--success))", colorTo: "hsl(var(--success))" }
    : isBlocked
    ? { colorFrom: "hsl(var(--destructive))", colorTo: "hsl(var(--destructive))" }
    : {};

  return (
    <Wrapper {...wrapperProps} containerClassName="w-full">
      <Card className={cn(
        "relative overflow-hidden border-2 transition-all duration-500 w-full rounded-xl",
        isGo ? "border-success/10 shadow-2xl shadow-success/10 bg-black" : isBlocked ? "border-destructive/10 shadow-2xl shadow-destructive/10 bg-black" : "border-canvas shadow-2xl bg-card"
      )}>
