          ...
          {localAnalysis && (
            <Card className="border-primary/20 bg-[#0b101e]">
              ...
              <CardContent className="space-y-5 text-sm">
                ...
                {localAnalysis.hard_filters.length > 0 && (
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-muted-foreground">Hard filters to check</p>
                    <ul className="mt-2 space-y-2">
                      {localAnalysis.hard_filters.map((filter, index) => (
                        <li key={`${filter.label}-${index}`} className="rounded-md border border-canvas bg-background/40 p-3">
                          <span className="font-semibold text-white">{filter.label}</span>
                          <span className="ml-2 text-xs uppercase tracking-widest text-muted-foreground">{filter.severity}</span>
                          <p className="mt-1 text-muted-foreground">{filter.evidence}</p>
                        </li>
                      ))}
                    </ul>
                  </div>
                )}
                {localAnalysis.required_skills.length > 0 && (
                  <div>
                    <p className="text-[10px] uppercase tracking-widest text-muted-foreground">Skills detected</p>
                    <div className="mt-2 flex flex-wrap gap-2">
                      {localAnalysis.required_skills.map((skill) => (
                        <span key={skill} className="rounded-md border border-primary/20 px-2 py-1 text-xs text-primary">
                          {skill}
                        </span>
                      ))}
                    </div>
                ...
                <div>
                  <p className="text-[10px] uppercase tracking-widest text-muted-foreground">Answer strategy</p>
                  <ul className="mt-2 list-disc space-y-1 pl-5 text-muted-foreground">
                    {localAnalysis.answer_strategy.map((item) => (
                      <li key={item}>{item}</li>
                    ))}
                  </ul>
                </div>
