...
description: Use this skill whenever you are adding, modifying, or auditing a sub-agent in Trajectory's src/trajectory/sub_agents/ folder, or when a task involves creating a new LLM-backed agent of any kind for this codebase. Triggers include phrases like "add a new agent", "new sub-agent", "phase 1 agent", "phase 4 generator", creating files in sub_agents/, editing AGENTS.md, or adding entries to _AGENT_REGISTRY in scripts/audit_prompt.py. Every sub-agent in this repo follows a strict 7-step pattern, and skipping any step produces silent bugs (wrong model, missing Content Shield tier classification, unregistered prompt audit, missing smoke test, wrong citation root). Do NOT use this skill for non-agent code (renderers, validators, bot handlers, scripts) or for editing prompts of agents that already exist and are registered.
---
