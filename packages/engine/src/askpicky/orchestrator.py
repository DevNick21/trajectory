...
def _truncate_headline(text: str, max_words: int = 12) -> str:
    """Truncate a headline to at most `max_words` words."""
    words = text.split()
...
def _triage_pass_headline(reasoning_brief: str) -> str:
    """Build a schema-safe headline for a deterministic triage skip."""
    reason = reasoning_brief.strip()
    reason_lower = reason.lower()
    if "too short" in reason_lower or "under 60" in reason_lower:
        return "Skip - JD is too short to evaluate."
    if not reason:
        return "Skip - triage found an obvious mismatch."
    return _truncate_headline("Skip - " + reason)
...
PHASE_1_AGENTS: list[str] = [
    ...
    # actually appear: local gov-data lookups first, then external fetches,
    # then LLM-backed risk checks).
    "companies_house",
    "sponsor_register",
    "soc_check",
    "gazette_check",
...
async def handle_forward_job(
    ...
    # A DeepSeek V4 Flash call (~$0.02) that gates whether the full $1-2 Phase 1
    ...
    if triage_result and triage_result.classification == "DEFINITE_PASS":
        ...
        verdict = Verdict(
            ...
            confidence_pct=95,
            entropy_norm=0.0,
            headline=_triage_pass_headline(triage_result.reasoning_brief),
            reasoning=[
                ReasoningPoint(
    ...
    # merge) was cut in the 2026-05-22 overhaul because it diluted Picky's
    # voice and made failure handling harder to reason about.
    ...
    # Every forward_job that produces a verdict creates a manual tracker row.
    # Best-effort: never fail the forward_job on a tracker write.
    try:
        from .applications import create_application_record
        ...
        log.warning("Failed to create tracker row for session %s: %s",
...
# ---------------------------------------------------------------------------
# Content Shield — bundle-wide wrapper
# ---------------------------------------------------------------------------
...
def _apply_rewrites_to_strings(obj: Any, rewrites: list[tuple[str, str]]) -> Any:
    ...
    been left out of scope. Add a regression test before revisiting.
    """
...
async def handle_draft_cv(
    ...
    # Phase 4 generators are high-stakes. Shield the bundle before
    # build_context so citation resolution uses the same redacted text the
    # model sees.
...
async def handle_draft_reply(
    ...
    # Pasted recruiter email is the primary injection vector; shield before
    # the high-stakes generator.
