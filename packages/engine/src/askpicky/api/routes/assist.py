...
from ...evaluators import evaluate_answer_claim_support
...
async def polish_answer(
    ...
    output.claim_support = evaluate_answer_claim_support(
        final_answer=output.final_answer,
        memory_suggestions=[
            suggestion
            for suggestion in suggestions
            if not output.memory_ids_used or suggestion.memory_id in output.memory_ids_used
        ],
    )
    unsupported_claims = [
        support.claim
        for support in output.claim_support
        if support.status == "unsupported"
    ]
    for claim in unsupported_claims:
        warning = f"Unsupported claim: {claim}"
        if warning not in output.missing_evidence_flags:
            output.missing_evidence_flags.append(warning)
