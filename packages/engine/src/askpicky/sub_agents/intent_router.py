...
Two-tier:
  - **Tier 0** (this file): deterministic URL + keyword rules. Catches
    ...
    fall through to:
  - **Tier 1**: DeepSeek V4 Flash via `call_agent`. System prompt from AGENTS.md §1.

12-way classification + the user has already pre-filtered ~80%
deterministically — the fast tier model handles the rest cleanly.
"""
...
# burning an LLM call. Last in the chain so genuine intents above
# take precedence.
_CHITCHAT_PATTERNS = [
...
async def route(
    ...
    # Tier 1 — DeepSeek V4 Flash. Only fires on ambiguous messages.
    # User messages are untrusted; run the cheap shield before routing.
