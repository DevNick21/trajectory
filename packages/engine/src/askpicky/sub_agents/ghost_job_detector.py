...
Combines 4 signals into a `GhostJobAssessment`:
  ...
  3. VAGUE_JD               — from the Ghost-Job JD Scorer (DeepSeek V4 Flash)
  4. COMPANY_DISTRESS       — from Companies House status + filings

Combination rules (source: product hard-blocker rules + test spec in
...
JD-scorer prompt below. A regex-only pass missed designer/creative JDs,
leadership JDs without bullet lists, and non-English-first sources. The fast
tier is the right ceiling for a signal that feeds confidence calibration, not a
hard blocker.
"""
...
_JD_SCORER_PROMPT = """Score a job description for how specific and real it sounds.
...
async def _score_jd(
    ...
    *,
    ...
    """5-dim JD specificity score via DeepSeek V4 Flash. ~$0.005, ~2-3s."""
