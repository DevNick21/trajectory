...
3. JSON-LD first, then `phase_1_jd_extractor`: extract ExtractedJobDescription.
4. Discover candidate company pages (careers / about / blog / values / team).
5. Fetch + clean each candidate.
6. `phase_1_company_scraper_summariser`: compress bounded advisory pages to
   CompanyResearch.

Company research must degrade gracefully. The original JD fetch is session
critical; secondary company pages are best-effort evidence with source-status
warnings, not a reason to crash the user flow.
...
_MAX_CANDIDATE_FIRECRAWL_FALLBACKS = 2
...
def _candidate_urls(company_domain: str) -> list[str]:
    ...
    return [f"{base}{p}" for p in _CANDIDATE_FETCH_PATHS]
...
async def _fetch_candidates(
    ...
    *,
    ...
    """Fetch bounded candidate company pages.

    Company research is advisory for the verdict. Keep this path bounded so
    a large public site cannot turn a single job forward into dozens of
    Playwright timeouts and Firecrawl fallback calls.
    """
    ...
    for url, r in zip(urls, results):
        if isinstance(r, str) and r.strip():
            ...
            if firecrawl_attempts >= _MAX_CANDIDATE_FIRECRAWL_FALLBACKS:
                logger.info("Skipping Firecrawl fallback for candidate page %s; limit reached", url)
                continue
...
async def _extract_jd(
    ...
    *,
    ...
    # JD extraction is low-stakes; run Tier 1 only. This replaces dangerous
    # patterns with [REDACTED: ...] markers inside the scraped text before it
    ...
    # Optional Tier 0 ground-truth block: when JSON-LD is present, the JD
    # extractor sees authoritative fields (datePosted, baseSalary) and should
    # prefer them over body-text inference.
    ...
    extracted = await call_agent(
        ...
        # (role, location, salary band, skills). DeepSeek V4 Flash handles
        # this cleanly. The JSON-LD
...
async def _summarise_company(
    *,
    ...
    # Company summarisation is low-stakes; Tier 1 is applied to each page's
    # text before it hits the prompt.
    from ..validators.content_shield import shield as shield_content
