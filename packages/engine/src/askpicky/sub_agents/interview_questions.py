...
module now:
  - `_build_user_input()` builds the shared role + company + career
    entries digest once.
  - `design()` calls DeepSeek V4 Flash with the question-designer system
    prompt and the QuestionSet output schema.
  - `predict()` calls DeepSeek V4 Flash with the likely-questions prompt and
    the LikelyQuestionsOutput schema (carries citations).

Both prompts live as separate markdown files in `prompts/`, loaded
on import. Both flows use DeepSeek V4 Flash (fast tier — question
generation is pattern-matching against a known JD + culture
