"""Optional onboarding reply parser.

The active web onboarding finalise path is deterministic and does not call
this module. Keep these per-stage parsers only for future inline
clarification UX where the user explicitly asks the app to interpret one
free-text stage before moving on.
...
User text is untrusted. Tier 1 Content Shield runs before the prompt is built.
Tier 2 doesn't: onboarding_parser is in the low-stakes agent list because its
tool-schema output cannot meaningfully leak into downstream generators.
...
async def _call_parser(
    *,
    ...
    # Fast tier is enough here: the parser does no reasoning, just
    # structured extraction from a short reply. Change routing only after
    # re-running the onboarding_parser smoke test.
