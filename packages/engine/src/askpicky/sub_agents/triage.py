...
A single DeepSeek V4 Flash call (~$0.02) classifies every forward_job as
...
def _fast_fail_classify(jd_text: str, user: UserProfile) -> Optional[TriageResult]:
    ...
    DeepSeek V4 Flash call. Zero cost, zero latency. Returns None when the case
    isn't obvious — caller proceeds to the LLM classifier."""

    # Empty or near-empty JD — could be a scraping failure on a
    # dynamic page (e.g. Oracle Cloud), not a genuinely short JD.
    # Fall through to the LLM classifier which has extracted
    # fields (role_title, skills, etc.) to make a better call.
...
async def classify(
    ...
    """Run the triage classifier: tier-0 deterministic first, then DeepSeek V4 Flash
