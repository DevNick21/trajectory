...
In those cases this module asks a cheap model (Haiku) to look at the
candidate set + whatever context we have (raw input, domain, scraped
page hints) and pick the most plausible trading entity — or refuse if
none look right.

Runs only on ~5-10% of resolutions (the genuinely ambiguous ones),
so the latency + cost overhead is bounded. Per CLAUDE.md Rule 7
this is a pattern-matching task, not judgement-heavy — Haiku is fine.
"""
...
def should_invoke_judge(
    *,
    ...
    """Heuristic: when should we spend the Haiku call?
