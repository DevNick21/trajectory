...
Multi-turn tool-use loop where the agent iteratively searches FAISS for
career entries as it drafts the CV. Uses provider-agnostic tool calling
via the OpenAI-compat API.

Two tools exposed to the agent:
  - `search_career_entries(query, kind_filter?, top_k?)` — semantic search
  - `get_user_profile_field(field)` — single-field profile lookup

Hallucination guard: every career_entry citation in the final CVOutput
must appear in `_retrieved_ids`.
"""
...
class CVTailorToolExecutor:
    ...
    async def _search(self, tool_input: dict) -> str:
        ...
        if remaining <= 0:
            return json.dumps({
                "error": f"retrieval budget exhausted ({_MAX_TOTAL_RETRIEVED} entries).",
                "results": [],
            })
...
async def generate(
    ...
    if cv is not None and _only_failure_is_hallucinated_citations(last_failures):
        store_ids = (
        ...
        if dropped:
            logger.warning(
                ...
                "post-retry rather than failing the whole draft.",
                dropped,
            )
            return cv
