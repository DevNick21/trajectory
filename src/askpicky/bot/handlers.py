...
async def on_outcome_callback(
    update: Update, context: ContextTypes.DEFAULT_TYPE,
) -> None:
    """Handle inline-keyboard taps from the Day-N outcome nudge.

    callback_data format: "ask:outcome:{session_id}:{outcome_kind}".
    Acknowledges the tap, records the outcome via the cross-surface
    `mark_outcome` helper, and replaces the keyboard with a confirmation.
    """
    ...
    data = query.data
    try:
        _, _, session_id, outcome_kind = data.split(":", 3)
    except ValueError:
        await query.answer("Bad button data — please report this.")
        return
    ...
    if user is None:
        await query.answer("Set up your profile on the web first.", show_alert=True)
        return
    ...
    # Defer to the shared cross-surface entry point. Imported lazily to
    # avoid the FastAPI dependency tree at bot import.
    from ..api.routes.outcomes import OutcomeKind, mark_outcome
    valid: tuple[str, ...] = (
        "applied", "no_response", "rejected_screen", "rejected_interview",
        "rejected_offer", "offer_received", "offer_accepted", "offer_declined",
    )
    if outcome_kind not in valid:
        await query.answer("Unknown outcome.", show_alert=True)
        return

    await mark_outcome(
        user=user, session=session, outcome=outcome_kind,  # type: ignore[arg-type]
    )
    confirmations = {
        "applied": "Got it — recorded as applied.",
        "no_response": "Logged — no response after waiting.",
        "rejected_screen": "Logged the rejection. That's the market, not you.",
        "rejected_interview": "Logged the rejection. Want an autopsy?",
        "rejected_offer": "Logged. We'll learn from this one.",
        "offer_received": "Nice — offer logged. Forward the letter for analysis.",
        "offer_accepted": "Congrats — recorded.",
        "offer_declined": "Logged. Reasoning matters; reply if you want to share why.",
    }
    await query.answer(confirmations.get(outcome_kind, "Logged."))
    # Replace the keyboard so the user can't double-tap.
    try:
...
async def on_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "Welcome to AskPicky. I verify roles before you apply — UK-first, "
        "visa-aware, no auto-apply.\n\n"
        "Set up your profile on the web first:\n"
        f"{settings.web_url}\n\n"
        "Takes a few minutes. Then forward me job URLs here."
    )
