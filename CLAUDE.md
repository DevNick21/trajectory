...
Package name: `askpicky`. Python imports are `from askpicky.X import Y`.
Frontend brand strings say "AskPicky" / "Picky".

Test-mode opt-out for required-secret validation: `ASKPICKY_TEST_MODE=1`.

---

## Hard architectural rules

These apply to every piece of code. A violating change must be revised until it
matches the rule.
...
Verdict.entropy_norm (0-1) measures evidence spread across signal pillars.
Motivation mismatch is a StretchConcern. A STRONG_GO or GO with any hard
blocker is a programmatic error; the guard flips it.
...
Every LLM call returns strict Pydantic-validated JSON from sub-agents.

### Rule 6 — On-demand generation
...
`MALICIOUS` + `REJECT` bails the pipeline with a user-visible message. The
Shield is a **precondition for agent invocation**.
...
|---------|------|-----------|
| Chat Completions (DeepSeek + OpenAI) | All agents | `chat.completions.create` with structured output (json_schema for OpenAI, json_object for DeepSeek) |
...
4. Pick the right adapter for any new agent. The adapter table above is the
   sanctioned dispatch surface.
